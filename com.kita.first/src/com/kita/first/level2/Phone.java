...
public class Phone { // public(접근제한자)가 붙을 수 있는 조건은 파일명과 같은 클래스만 붙을 수 있다.
	...
	// 생성자는 리턴타입이 존재하지 않는다.
	Phone() {} // 기본 생성자. (사용자가 아무것도 만들지 않았을 땐 컴파일러가 기본생성자를 자동 생성해줌) 
	
	// 17~20라인 처럼 this()로 받지 않는 생성자가 하나는 존재 해야한다.
	Phone(String name, String color, int memory) { // 필드를 초기화 시켜주는 역할 // 변수의 이름을 정할땐 의미가 있게 짓자.
	...
	// 오버로딩 가능, (같은 생성자 이름의 매개변수의 길이,타입이 다르면 가능)
	Phone(String name){
		this(name,"블랙",200); // this()는 생성자의 첫번 째 라인에서만 선언이 가능하다
	}
